...
type Catalog struct {
	...
	Price      float64
	Brand      string `tigris:"searchIndex,facet"`
	Labels     string `tigris:"searchIndex,facet"`
	Popularity int
...
Fields behavior can be modified and enriched using `tigris` struct tag.

[Check available tags and example usage.](../types/#field-tags).
