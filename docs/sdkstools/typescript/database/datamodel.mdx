...
With the Tigris TypeScript client you can define your data models using **Decorated class**
or **declare-using-interface**.
...
When using classes to model your collection with nested arrays, additional `depth` option is required
in `@Field()` annotation. `depth` identifies the depth of your nested array. E.g - field of
...
If you prefer to not use class decorators you can define the collection schema as follows:
